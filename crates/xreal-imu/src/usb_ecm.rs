...
pub fn connect_usb() -> Result<mpsc::UnboundedReceiver<Vec<u8>>> {
    ...
    // 3. Discover CDC ECM interfaces and endpoints from descriptors.
    let config = device
    ...
    // Activate bulk endpoints (CDC ECM Data interface alt setting 1).
    if ecm.needs_alt_setting {
        data.set_alt_setting(ecm.data_alt_setting)
            .wait()
            .map_err(|e| anyhow!("Failed to activate CDC ECM data endpoints: {e}"))?;
        tracing::debug!(alt = ecm.data_alt_setting, "Activated data interface alt setting");
    }

    // 5. Enable packet reception via SET_ETHERNET_PACKET_FILTER.
    if let Err(e) = comm.control_out(
        ...
        tracing::warn!(?e, "SET_ETHERNET_PACKET_FILTER failed (may still work)");
    }

    // 6. Create bulk endpoint reader/writer (std::io::Read / Write).
    let reader = data
...
fn find_ecm_endpoints(config: &nusb::descriptors::ConfigurationDescriptor) -> Result<EcmEndpoints> {
    ...
    for iface_desc in config.interface_alt_settings() {
        ...
        if class == 0x02 && (subclass == 0x06 || subclass == 0x0D) {
            ...
            let proto = if subclass == 0x06 { "ECM" } else { "NCM" };
            tracing::debug!(iface_num, alt, proto, "Found CDC Communication interface");
        }
...
fn ecm_tcp_thread(
    ...
    tracing::info!("TCP SYN sent to XReal One IMU via USB CDC ECM");
